...
def quick_train():
    ...
    print(f"訓練集大小: {len(train_texts)} (子集)")
    print(f"測試集大小: {len(test_texts)} (子集)")
    
    # TF-IDF向量化 (較小的特徵集)
    print("進行TF-IDF向量化...")
    vectorizer = TfidfVectorizer(
        max_features=5000,  # 減少特徵數
        ngram_range=(1, 2),
        min_df=2,
        max_df=0.95
    )
    ...
    models = {
        ...
        'svm': SVC(kernel='linear', random_state=42, probability=True),  # 使用線性核
        'random_forest': RandomForestClassifier(n_estimators=50, random_state=42)  # 減少樹數
    }
    ...
    with open('experiments/results/quick_training_results.pkl', 'wb') as f:
        pickle.dump({
            'results': results,
            'subset_info': {
                'train_size': len(train_texts),
                'test_size': len(test_texts),
                'feature_size': train_vectors.shape[1]
            }
        }, f)
