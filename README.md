...
本專案為深度學習練習專案，使用IMDB Movie Reviews資料集進行情感分析。專案涵蓋從傳統機器學習到現代Transformer模型的完整技術棧，提供端到端的NLP解決方案。
...
| 類型 | 模型 | 實際準確率 | F1分數 | AUC | 訓練時間 | 過擬合程度 |
|------|------|-----------|--------|-----|----------|------------|
| 傳統ML | **邏輯回歸 + TF-IDF** | **76.83%** | **0.771** | **0.851** | 0.1s | 低 (7.0%) |
| 傳統ML | 樸素貝葉斯 + TF-IDF | 76.66% | 0.769 | 0.852 | <0.1s | 低 (6.7%) |
| 傳統ML | 隨機森林 + TF-IDF | 73.30% | 0.738 | 0.804 | 6.4s | 高 (25.8%) |

> **完整資料集訓練結果**: 
> - 原始資料: 50,000筆IMDB電影評論
> - 預處理後: 49,283筆 (訓練集39,283筆，測試集9,821筆)  
> - 特徵向量: TF-IDF (15,000維，1-2gram)
> - **真實完整資料集結果** - 非子集測試
...
| 處理步驟 | 效果 |
|---------|------|
| HTML標籤清理 | 移除所有`<br/>`、`<b>`等標籤 |
| 文本正規化 | 轉小寫、統一編碼 |
| 否定詞處理 | "not good" → "not_good" |
| 詞彙化處理 | "running" → "run" |
| 長度過濾 | 移除<10字元的評論 |
| **平均長度縮減** | **89.6%** (1,309→135字元) |
| **處理時間** | **35.4秒** (50,000筆資料) |
